...
ifdef k512      ;Initialise memory model for 512K board
    ...
    out ($7a),a
    ld a,$24          ;Map BASIC RAM xxROMxx into bank 3
    out ($7b),a
endif
    
    call SERIAL_INIT        ;Initialise serial port
 
call SERIAL_INLINE
defb "Booting...",13,10,0
    
        di                        ;{{0591:f3}} 

    ld hl,term_boot             ;Boot into terminal
    ld c,$fc                    ;Enable upper and lower ROMs
    jp MC_START_PROGRAM
...
boot_message:                     ;{{Addr=$0688 Data Calls/jump count: 0 Data use count: 1}}
        ...
        defb "Copyright"          
        defb $1f,$02,$04          
        defb "(C)"                  ; copyright symbol
        defb "1985 Amstrad Consumer Electronics"
        defb $1f,$0c,$05          
