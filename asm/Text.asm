...
push hl
ld hl,TERM_CURSORPOS            ;We want return to pass via TERM _CURSORPOS
ex (sp),hl
...
_txt_cur_enable_1:                ;{{Addr=$1288 Code Calls/jump count: 2 Data use count: 0}}
    ...
    push af
    ld hl,cursor_visible
    call TERM_ESC_ASCIIZ
    pop af
...
TXT_SET_PEN_:                     ;{{Addr=$12a6 Code Calls/jump count: 1 Data use count: 2}}
    push de
    ld d,a
    ld hl,set_foreground_colour
    call TERM_ESC_ASCIIZ
    ld a,d
    call cpc_ink_to_serial_ink
    ld e,a
    call TERM_DECIMAL
    ld a,'m'
    call SERIAL_OUT
...
;;==
;Convert a CPC pen/paper (ink) number to a VT100 colour 
;(work in progress on look up table)
cpc_ink_to_serial_ink:
    push hl
    ld hl,cpc_ink_to_serial_ink_table
    add a,l
    ld l,a
    jr nc,cpc_ink_no_carry
    inc h
cpc_ink_no_carry:
    ld a,(hl)
    pop hl
...
cpc_ink_to_serial_ink_table:
    defb 4,11,9,0,1,2,3,5,6,7,8      ;Mapping between CPC colours (pen/paper) and VT100 colours
    defb 10,12,13,14,15
...
control_code_handler_functions:   ;{{Addr=$1474 Data Calls/jump count: 0 Data use count: 1}}
        ...
        defb $80                  
        defw performs_control_character_NUL_function;&00 0 NUL: ##LABEL##
        defb $81                  
        defw TXT_WR_CHAR          ;&01 SOH: firmware function: TXT WR CHAR ##LABEL##
        defb $80                  
        defw TXT_CUR_DISABLE      ;&02 STX: firmware function: TXT CUR DISABLE ##LABEL##
        defb $80                  
...
TXT_CLEAR_WINDOW:                 ;{{Addr=$154f Code Calls/jump count: 0 Data use count: 2}}
    ...
    ld de,$324A         ;Escape command 2J - CLS
    call TERM_ESC_DE
