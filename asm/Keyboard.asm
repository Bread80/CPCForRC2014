...
;C=Column and shift/control state??
;B=Row???
...
;;=====================================
;Read the serial port and push any characters found into the key buffer
;Out: AF corrupt
serial_to_buffer:
    call SERIAL_READ        ;Read char from serial port
    ret z                   ;No char read

    push hl
    push bc

    call serial_codes_to_cpc_keys
    
    call search_key_tables  ;Convert char to key code
;call nc,output_as_hex
    call c,push_into_key_buffer ;Push key code into key buffer
    pop bc
    pop hl
    jr serial_to_buffer     ;Loop to empty serial port buffer

;Map characters and escape sequences from the serial port to
;CPC characters.
...
escape_square_sequence:     ;Test for n in ESC[n~
    call SERIAL_READ
    jr z,escape_square_sequence
    
    ld hl,escape_square_to_cpc_table
    cp '0'
    jr c,escape_table_lookup    ;Normal escape code
    cp '9'+1
    jr nc,escape_table_lookup   ;Normal escape code
    
    ld b,a                  ;Preserve parameter (n)
escape_tilde_loop           ;ESC[n~ sequence
    call SERIAL_READ        ;Read possible tilde
    jr z,escape_tilde_loop

    cp '~'
    ld a,b                  ;Retrieve parameter (n)
    ld hl,escape_tilde_to_cpc_table ;Tilde lookup table
    jr z,escape_table_lookup
    ret                     ;Not a recognised sequence. We'll return 'n'
    
escape_O_sequence:
    call SERIAL_READ
    jr z,escape_O_sequence
...
escape_mapping_loop:
    inc hl
    cp (hl)
    jr z,escape_mapping_found
    inc hl
    djnz escape_mapping_loop
    
    ret                     ;No mapping - return input character
    
escape_mapping_found:
    inc hl                  ;Read mapped char
    ld a,(hl)
    ret
...
its_a_number
    jp search_key_tables

;Search through all key tables to convert ASCII char to a key value
;In:    A=ASCII char
;Out:   Carry set if key found
;       B=column (0-7) in key tables - bitwise value, B7 to B0. 
;       C=Row (0-10); Bit 5=Shift state, Bit 7=Control state
;       If no key found, A preserved,,otherwise A corrupt
;       HL, Flags corrupt
search_key_tables:
    ld hl,(address_of_the_normal_key_table)
    call search_key_table   ;Search normal table
    ret c
    
    call search_key_table   ;Search shifted table
    jr nc,key_not_found
    set 5,c
    ret
    
key_not_found:
    call search_key_table   ;Search control table
    ret nc
    set 7,c
    ret
    
;Search a single key table to convert an ASCII value to a key
;In:    HL=address of a key table, A=ASCII char
;Out:   Carry set key found
;       B,C = Column, row data (no shift/control data in C) (Corrupt if no key found)
;       If key not found, HL = address of byte after end of table, otherwise HL corrupt
...
keyboard_translation_tables:      ;{{Addr=$1eef Data Calls/jump count: 0 Data use count: 1}}
        defb $f0,$f3,$f1,$89,$86,$83,$8b,$8a    ;Cur U	Cur R	Cur D	f9	f6	f3	Enter	f.
        defb $f2,$e0,$87,$88,$85,$81,$82,$80    ;Cur L	Copy	f7	f8	f5	f1	f2	f0
        defb $10,$5b,$0d,$5d,$84,$ff,$5c,$ff    ;Clr	[	Ret	]	f4		\
        ...
        defb $31,$32,$fc,$71,$09,$61,$fd,$7a    ;1	2	Esc	q	Tab	a	Caps	z
        defb $0b,$0a,$08,$09,$58,$5a,$ff,$7f    ;[VT]	[LF]	[BS]	[TAB]	Fire2	Fire1		Del

;Shifted key table
        defb $f4,$f7,$f5,$89,$86,$83,$8b,$8a    ;Cur U	Cur R	Cur D	f9	f6	f3	Enter	f.
        defb $f6,$e0,$87,$88,$85,$81,$82,$80    ;Cur L	Copy	f7	f8	f5	f1	f2	f0
        defb $10,$7b,$0d,$7d,$84,$ff,$60,$ff    ;Clr	[	Ret	]	f4		\
        ...
        defb $21,$22,$fc,$51,$09,$41,$fd,$5a    ;1	2	Esc	q	Tab	a	Caps	z
        defb $0b,$0a,$08,$09,$58,$5a,$ff,$7f    ;[VT]	[LF]	[BS]	[TAB]	Fire2	Fire1		Del
