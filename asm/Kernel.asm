...
ifdef flat
    ifdef mem_base
        ...
        ld      de,$4000            ;Flat memory model - RAM starts at &4000
    endif
else
...
;call SERIAL_INLINE             ;Uncomment for debugging
;defb 13,10,"Launching BASIC...",13,10,0
;halt
        rst     $18               ;{{0095:df}}  RST 3 - LOW: FAR CALL
        defw entry_point_of_foreground_ROM_in_use_                
;call SERIAL_INLINE
;defb "Return from BASIC...",13,10,0
...
KL_ROM_WALK:                      ;{{Addr=$0326 Code Calls/jump count: 0 Data use count: 1}}
    ...
    call term_install       ;Install CPC for RC2014 terminal. It will return adjusted HL (last byte of free memory)
